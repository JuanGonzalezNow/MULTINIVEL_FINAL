        ...
        try:
            if opcion == "1":
                ...
                if clase not in ["Guerrero", "Mago", "Arquero"]:
                    print("Clase no válida. Selección cancelada.")
                    continue
                nivel = int(input("Nivel inicial: "))
                controlador.crear_personaje(nombre, clase, nivel)
                print("¡Personaje creado e insertado en MySQL con éxito!")
                ...
                if not personajes:
                    ...
                    for p in personajes:
                        ...
                        print(f" -> Acción: {p.habilidad_especial()}")
                ...
                print("Cerrando el gestor RPG Manager. ¡Éxito en la sustentación!")
                break
            else:
                print("Opción inválida. Digite un número entre 1 y 6.")
                
        except ValueError as ve:
            print(f"Error de entrada: Los campos numéricos de nivel e ID deben ser enteros válidos. ({ve})")
